...
void ChatLogic::LoadAnswerGraphFromFile(std::string filename)
{
        ...
        while (getline(file, lineStr))
        {
            ...
            if (type != tokens.end())
            {
                ...
                if (idToken != tokens.end())
                {
                    ...
                    if (type->second == "NODE")
                    {
                        // check if node with this ID exists already in vector of exclusively owned node handles
                        auto newNode = std::find_if(_nodes.begin(), _nodes.end(), [&id](std::unique_ptr<GraphNode> const &node) { return node->GetID() == id; });

                        // create new element if ID does not yet exist
                        if (newNode == _nodes.end())
                        {
                            // create chatlogic exclusively owned node pointer and push it into _nodes vector
                    ...
                    if (type->second == "EDGE")
                    {
                        ...
                        {
                            // get iterator on incoming and outgoing node via ID search in vector of exclusively owned nodes
                            ...
                            // create new exclusively owned edge
                            std::unique_ptr<GraphEdge> edge = std::make_unique<GraphEdge>(id);

                            // set parent and child node for the newly created edge
                            ...
                            // move exclusively owned edge by chatlogic into node that has this edge as outgoing
    ...
    // create local instance of chatbot on stack
    ChatBot localBot("../images/chatbot.png");
    // set _chatlogic handle to this instance for now
    localBot.SetChatLogicHandle(this);
    // set root node for the locally created chatbot
    localBot.SetRootNode(rootNode);
    // move locally created chatbot into rootNode
